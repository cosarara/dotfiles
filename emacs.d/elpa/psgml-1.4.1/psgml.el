...
and move to the line in the SGML document that caused it."
  ...
  ;; next form has been migrated from                compile-internal to       compilation-start
  ;;                                                    (old)                     (new):
  (compilation-start command                         ;; COMMAND                   COMMAND
		     nil                             ;; -                         MODE
		     nil                             ;; NAME-FUNCTION             NAME-FUNCTION
		     sgml-validate-error-regexps))   ;; REGEXP-ALIST              HIGHLIGHT-REGEXP


  ;; (NAME-FUNCTION was implicitly nil in the old version.)

  ;; Remarks on 'disappeared' arguments:

  ;; 		     "No more errors" (old: ERROR-MESSAGE) dropped, as compilation-start has no such arg.

  ;;                 The old PARSER argument is no longer there and was nil anyway.

  ;;                 Old NAME-OF-MODE was "SGML validation", but new MODE seems to want a symbol,
  ;;                 not a string, presumably a real, existing, defined mode. nil works fine
